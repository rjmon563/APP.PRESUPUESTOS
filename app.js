...
// ==========================================
// 2. NAVEGACIÓN Y CLIENTES (CON CP Y BORRAR)
// ==========================================
...
window.guardarDatosCliente = () => {
    ...
    db.clientes.push({ 
        ...
        cp: document.getElementById('cli-cp').value.toUpperCase(), // Guardamos el CP
        presupuestos: [] 
    });
    asegurarGuardado();
...
window.borrarCliente = (id) => {
    if (confirm("¿BORRAR ESTE CLIENTE Y TODOS SUS PRESUPUESTOS?")) {
...
// ==========================================
// 4. CALCULADORA (CON BORRADO INTELIGENTE)
// ==========================================
window.teclear = (n) => {
    if (n === '+') {
        let vP = parseFloat(calcEstado.memoria.replace(',', '.')) || 0;
        if (vP !== 0) { calcEstado.acumulado += vP; calcEstado.memoria = ''; }
        actualizarDisplay();
    } else if (n === 'OK') {
        ...
        // Si hay algo escribiéndose, borra eso. Si no, borra toda la suma acumulada.
...
// ==========================================
// 5. RENDER Y PDF (CON EDITOR DE TOTAL)
// ==========================================
