...
window.renderListaClientes = () => {
    ...
    db.clientes.map(c => `
        <div onclick="abrirExpediente(${c.id})" class="bg-white p-5 rounded-[30px] border shadow-sm flex justify-between items-center mb-3 active-scale">
            ...
            <button onclick="event.stopPropagation(); if(confirm('¿Borrar?')){db.clientes=db.clientes.filter(x=>x.id!==${c.id}); asegurarGuardado(); renderListaClientes();}" class="text-red-500 p-3 rounded-2xl bg-red-50">🗑️</button>
        </div>`).reverse().join('');
};

window.abrirExpediente = (id) => {
    clienteActual = db.clientes.find(x => x.id === id);
    document.getElementById('ficha-cliente-detalle').innerHTML = `<div class="bg-blue-600 text-white p-7 rounded-[40px] italic"><h2 class="text-xl font-black uppercase mb-1">${clienteActual.nombre}</h2><p class="text-[10px] opacity-80 uppercase">${clienteActual.dir}</p></div>`;
...
function renderBotones() {
    document.getElementById('botones-trabajo').innerHTML = Object.keys(CONFIG_MEDIDAS).map(k => `
        <button onclick="prepararMedida('${k}')" class="bg-white p-6 rounded-[30px] border flex flex-col items-center active-scale">
...
function renderMedidas() {
    ...
    cont.innerHTML = obraEnCurso.lineas.map(l => `
        <div class="bg-white p-4 rounded-2xl border flex justify-between items-center mb-2 font-bold text-[10px] uppercase italic">
        ...
        (subtotal > 0 ? `<div class="bg-slate-900 text-white p-6 rounded-[35px] mt-5 italic"><div class="flex justify-between text-[10px] opacity-60 font-black"><span>Base: ${fNum(subtotal)}€</span><span>IVA (${obraEnCurso.iva}%): ${fNum(cuotaIva)}€</span></div><div class="flex justify-between text-xl font-black text-green-400 border-t border-white/10 pt-2"><span>TOTAL:</span><span>${fNum(total)}€</span></div></div>` : '');
}
...
function renderGaleria() {
    ...
    cont.innerHTML = obraEnCurso.fotos.map((f, i) => `<div class="relative min-w-[80px] h-20"><img src="${f}" class="w-20 h-20 object-cover rounded-xl border"><button onclick="obraEnCurso.fotos.splice(${i},1); renderGaleria();" class="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-5 h-5 text-[10px]">✕</button></div>`).join('');
}
