...
// Formateador de números (Estilo España)
...
// ==========================================
// 3. CALCULADORA (COMA DECIMAL Y SUMA INFINITA)
// ==========================================
window.teclear = (n) => {
    if (n === '+') {
        ...
        // Tratamos el punto como coma visual
...
// ==========================================
// 4. GESTIÓN DE MEDICIONES Y TOTALES
// ==========================================
...
window.editarLinea = (id) => {
    ...
    calcEstado = { 
        tipo: l.tipo, paso: 1, v1: l.cantidad, v2: 0, 
        memoria: l.precio.toString().replace('.', ','), 
        acumulado: 0, zona: l.zona, tarea: l.tarea, 
        modo: 'precio', totalMetros: l.cantidad, editandoId: id 
    };
    abrirCalculadora();
};
...
window.guardarObraCompleta = async () => {
    ...
    const opt = { 
        margin: 10, 
        filename: `Presu_${db.ajustes.nPresu}_${obraEnCurso.nombre}.pdf`, 
        html2canvas: { scale: 2 }, 
    ...
    try {
        ...
        if (navigator.share) {
            await navigator.share({
                ...
                text: 'Hola, le adjunto el presupuesto solicitado.'
            });
        } else {
...
// ==========================================
// 6. GESTIÓN DE CLIENTES Y EXPEDIENTES
// ==========================================
        ...
        <div class="space-y-2 mb-4">
            ...
            ${historial.map(p => `
                <div class="bg-white p-4 rounded-2xl border shadow-sm flex justify-between items-center">
                    <div>
                        <p class="text-[10px] font-bold">#${p.numero} - ${p.nombreObra}</p>
                        <p class="text-[8px] opacity-40">${p.fecha}</p>
                    </div>
...
window.prepararMedida = (t) => {
    ...
    if (!tarea) return;
    calcEstado = { 
        tipo: t, paso: 1, v1: 0, v2: 0, memoria: '', acumulado: 0, 
        zona: zona.toUpperCase(), tarea: tarea.toUpperCase(), 
        modo: 'medida', editandoId: null 
    };
    abrirCalculadora();
};
