...
// 4. CALCULADORA MEJORADA (SUMA INFINITA)
window.teclear = (n) => {
    if (n === '+') {
...
function actualizarDisplay() {
    ...
    if (calcEstado.acumulado > 0) {
        display.innerHTML = `<span class="text-sm opacity-50 font-normal">Suma: ${fNum(calcEstado.acumulado)} +</span><br>${visual}`;
    } else {
...
window.prepararMedida = (t) => {
    const zona = prompt("¿HABITACIÓN / ZONA?", "GENERAL"); 
    if (!zona) return;
...
window.guardarAjustes = () => {
    ...
    asegurarGuardado(); alert("✅ Configuración guardada"); irAPantalla('clientes');
};
...
window.renderMedidas = () => {
    ...
    cont.innerHTML = obraEnCurso.lineas.map(l => `
        <div class="bg-white p-4 rounded-2xl border flex justify-between items-center mb-2 font-bold text-[10px] uppercase italic shadow-sm">
            ...
            <div class="flex items-center gap-1"><span class="font-black text-xs mr-2">${fNum(l.subtotal)}€</span><button onclick="borrarLinea(${l.id})" class="text-red-400 p-2 rounded-xl bg-red-50">✕</button></div>
        </div>`).reverse().join('') + 
...
window.renderBotones = () => {
    ...
    cont.innerHTML = Object.keys(CONFIG_MEDIDAS).map(k => `
        <button onclick="prepararMedida('${k}')" class="bg-white p-6 rounded-[30px] border flex flex-col items-center active-scale shadow-sm">
...
window.borrarLinea = (id) => { if(confirm("¿Eliminar esta línea?")) { obraEnCurso.lineas = obraEnCurso.lineas.filter(x => x.id !== id); renderMedidas(); } };
...
window.renderListaClientes = () => {
    ...
    db.clientes.map(c => `<div onclick="abrirExpediente(${c.id})" class="bg-white p-5 rounded-[30px] border shadow-sm flex justify-between items-center mb-3 active-scale"><p class="font-black text-slate-800 uppercase italic text-sm">${c.nombre}</p><button onclick="event.stopPropagation(); if(confirm('¿Borrar cliente?')){db.clientes=db.clientes.filter(x=>x.id!==${c.id}); asegurarGuardado(); renderListaClientes();}" class="text-red-500 p-3 rounded-2xl bg-red-50">🗑️</button></div>`).reverse().join('');
};
...
window.confirmarNombreObra = () => {
    ...
    if (!v) return alert("Escribe un nombre para el presupuesto");
...
window.guardarObraCompleta = async () => {
    if (obraEnCurso.lineas.length === 0) return alert("Añade alguna medida primero");
    // Lógica de PDF simplificada para asegurar funcionamiento
    const numFactura = `${new Date().getFullYear()}/${String(db.ajustes.nPresu).padStart(3, '0')}`;
    alert("Generando PDF para " + clienteActual.nombre);
    db.ajustes.nPresu++; 
    asegurarGuardado(); 
    irAPantalla('expediente');
};
