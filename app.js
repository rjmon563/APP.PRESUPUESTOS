...
// ==========================================
// 3. EXPEDIENTE E HISTORIAL (REPARADO)
// ==========================================
window.abrirExpediente = (id) => {
    ...
    document.getElementById('ficha-cliente-detalle').innerHTML = `
        ...
        <div class="space-y-2 mb-4">
            <p class="text-[9px] font-black opacity-40 ml-2 uppercase">Historial (Pulsa para abrir)</p>
            ${historial.map(p => `
                <div onclick="verPresupuestoGuardado(${p.id})" class="bg-white p-4 rounded-2xl border shadow-sm flex justify-between items-center active:scale-95 transition-all cursor-pointer">
                    <div>
...
window.verPresupuestoGuardado = (idPresu) => {
    ...
    if (!p) return alert("No se encuentra el presupuesto");
    
    obraEnCurso = {
        nombre: p.nombreObra,
        lineas: JSON.parse(JSON.stringify(p.lineas)),
        iva: p.iva || 21,
        fotos: []
    };
...
// ==========================================
// 4. TRABAJO Y MEDICIONES (INCLUYE EDITOR)
// ==========================================
...
window.guardarObraCompleta = async () => {
    ...
    const nuevoPresu = {
        ...
        fecha: fechaManual ? new Date(fechaManual).toLocaleDateString() : new Date().toLocaleDateString(),
    ...
    try {
        ...
        if (navigator.share) {
            await navigator.share({ files: [file], title: 'Presupuesto', text: 'Adjunto presupuesto.' });
        } else {
