...
function inyectarLogo() {
    ...
    if (header) {
        header.innerHTML = `
            <div style="display:flex; align-items:center; gap:12px; padding:15px; background:white; border-bottom:1px solid #eee; margin-bottom:10px;">
                <img src="${LOGO_BASE64}" style="width:40px; height:40px; border-radius:8px;">
                <span style="font-weight:900; font-style:italic; color:#1e293b; text-transform:uppercase;">PRESUPRO V3</span>
            </div>`;
    }
}
...
window.abrirExpediente = (id) => {
    ...
    document.getElementById('ficha-cliente-detalle').innerHTML = `
        ...
        <div class="bg-yellow-100 p-5 rounded-[30px] mb-4 border border-yellow-200">
            ...
            <textarea oninput="guardarNotas(${clienteActual.id}, this.value)" class="w-full bg-transparent border-none outline-none font-bold text-sm h-20 resize-none" placeholder="Anotaciones...">${clienteActual.notas || ""}</textarea>
        </div>
        <div class="space-y-2 mb-4">${historial.map(p => `
            <div class="flex items-center gap-2 mb-2">
                <div onclick="verPresupuestoGuardado(${p.id})" class="flex-1 bg-white p-4 rounded-2xl border shadow-sm flex justify-between items-center active:scale-95 border-l-4 border-l-blue-400">
                ...
                <button onclick="borrarPresupuestoIndividual(${p.id})" class="bg-red-50 text-red-500 p-4 rounded-2xl border border-red-100">🗑️</button>
            </div>`).reverse().join('') || '<p class="text-center opacity-30 text-[10px] py-4">Sin presupuestos</p>'}</div>`;
    irAPantalla('expediente');
};

// ==========================================
// 5. RENDER MEDICIONES (CON DESGLOSE RECUPERADO)
// ==========================================
window.renderMedidas = () => {
    ...
    const totalCalculado = subtotal + cuotaIva;

    let htmlLineas = obraEnCurso.lineas.map(l => `
    ...
    let htmlTotales = (subtotal > 0) ? `
        <div class="bg-white p-6 rounded-[35px] mt-5 italic shadow-md border">
            <div class="flex justify-between text-[11px] font-bold mb-1"><span class="opacity-50 uppercase">Base Imponible:</span><span>${fNum(subtotal)}€</span></div>
            <div class="flex justify-between text-[11px] font-bold mb-1 text-blue-600"><span class="uppercase">IVA (${obraEnCurso.iva}%):</span><span>${fNum(cuotaIva)}€</span></div>
            <div class="flex justify-between text-[13px] font-black border-t pt-2 mt-2"><span>TOTAL CALC.:</span><span>${fNum(totalCalculado)}€</span></div>
        </div>
        <div class="bg-slate-900 text-white p-6 rounded-[35px] mt-4 italic shadow-xl border-l-4 border-l-green-500">
            <p class="text-[8px] text-green-400 font-black uppercase mb-1">✍️ Editar Total Final (si quieres redondear):</p>
            <input type="text" id="total-editable" class="w-full bg-transparent text-white text-2xl font-black border-b border-white/20 outline-none" value="${fNum(totalCalculado)}">
        </div>` : '';

    cont.innerHTML = htmlLineas + htmlTotales;
};

// ==========================================
// 6. FUNCIONES RESTANTES (MANTENIDAS)
// ==========================================
...
window.prepararMedida = (t) => { const zona = prompt("¿ZONA?"); if (!zona) return; calcEstado = { tipo: t, paso: 1, v1: 0, v2: 0, memoria: '', acumulado: 0, zona: zona.toUpperCase(), tarea: 'MONTAJE', modo: 'medida', editandoId: null, historialSuma: [] }; abrirCalculadora(); };
...
window.guardarAjustes = () => { db.ajustes = { nombre: document.getElementById('config-nombre').value.toUpperCase(), cif: document.getElementById('config-cif').value.toUpperCase(), tel: document.getElementById('config-tel').value, nPresu: parseInt(document.getElementById('config-nPresu').value) || 1 }; asegurarGuardado(); irAPantalla('clientes'); };
window.editarLinea = (id) => { const l = obraEnCurso.lineas.find(x => x.id === id); if (!l) return; calcEstado = { tipo: l.tipo, paso: 1, v1: 0, v2: 0, memoria: l.cantidad.toString().replace('.', ','), acumulado: 0, zona: l.zona, tarea: l.tarea, modo: 'medida', totalMetros: l.cantidad, editandoId: id, historialSuma: [] }; abrirCalculadora(); };
